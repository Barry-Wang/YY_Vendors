...
- (void)prepare
{
    [super prepare];
     ...
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(netWorkStatusChange) name:kNetWorkStatusChanged object:nil];
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(netWorkStatusChange) name:@"kNoNetWorkStatus" object:nil];
    // 初始化文字
    //[self setTitle:@"请检测网络" forState:MJRefreshStateIdle];
...
- (void) netWorkStatusChange {
    ...
    if ([WYNetworkingManager shareInstance].reachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
